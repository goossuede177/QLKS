...
namespace Qlks
{
    class CauLenh
    {
        SqlConnection con = new SqlConnection(@"Data Source=KHOA-PC\SQLEXPRESS;Initial Catalog=TTQLKS;Integrated Security=True");
        ...
        public DataTable suavatdung(string b,string g, string tv, string tl, string gi,string mst)
        {
            if (con.State == ConnectionState.Open)
                con.Close();
            con.Open();

            SqlCommand cm = new SqlCommand("suavatdung", con);
            cm.CommandType = CommandType.StoredProcedure;
            cm.Parameters.AddWithValue("@ban", b);
            cm.Parameters.AddWithValue("@ghe", g);
            cm.Parameters.AddWithValue("@tivi", tv);
            cm.Parameters.AddWithValue("@tulanh", tl);
            cm.Parameters.AddWithValue("@giuong", gi);
            cm.Parameters.AddWithValue("@maysaytoc", mst);
        ...
        public DataTable hienvatdung(int ma)
        {
            if (con.State == ConnectionState.Open)
                con.Close();
            con.Open();

            SqlCommand cm = new SqlCommand("hienvatdung", con);
            cm.CommandType = CommandType.StoredProcedure;
